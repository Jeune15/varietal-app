...
import { Plus, Trash2, Search, Filter, DollarSign, FileText, Calendar, CheckCircle, X } from 'lucide-react';
...
export const ExpensesView = () => {
  ...
  return (
    <div className="h-full flex flex-col gap-6 animate-fade-in">
      ...
      {showModal && (
        <div 
          className="fixed inset-0 z-[100] bg-white/90 backdrop-blur-sm flex items-center justify-center p-4"
          onClick={() => setShowModal(false)}
        >
          <div 
            className="bg-white w-full max-w-md border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-0 animate-in fade-in zoom-in duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex justify-between items-center p-6 border-b border-black">
              <h3 className="text-xl font-black text-black uppercase tracking-tight">Registrar Nuevo Gasto</h3>
              <button onClick={() => setShowModal(false)} className="hover:bg-stone-100 p-1 transition-colors">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-3 bg-stone-50 border border-stone-200 focus:border-black focus:ring-0 transition-all font-medium text-sm"
                    required
                  />
                </div>
                <div>
              ...
              <div className="pt-4 border-t border-stone-100 flex justify-end gap-3">
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="px-6 py-3 border border-stone-200 text-stone-500 font-bold uppercase tracking-wider hover:text-black hover:border-black transition-all text-xs"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="px-6 py-3 bg-black text-white border border-black font-bold uppercase tracking-wider hover:bg-stone-800 transition-all text-xs"
                >
                  Guardar Gasto
                </button>
              </div>
